...
from libcity.data.utils import generate_dataloader, generate_dataloader_mixup, generate_dataloader_subset_aug, generate_dataloader_aug
...
class AirQualityDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.input_window = self.config.get('input_window', 24)
        self.output_window = self.config.get('output_window', 12)
        self.output_offset = self.config.get('output_offset', 1)
        self.loss_type = self.config.get('loss_type', 'ERM')
        self.weight_file = self.config.get('weight_file', 'none')
        self.group_file = self.config.get('group_file', 'none')
        self.timesolts = []
        self.geo_coords = []
        ...
        # 使用的是否是增强的数据
        self.augmented_data = self.config.get('augmented_data', False)
        self.aug_params = self.config.get('aug_params', {})
        self.aug_file = self.config.get('aug_file', None)
        self.aug_parameters_str = None
        if self.augmented_data:
            self.aug_parameters_str = \
                str(self.dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' \
                + str(self.train_rate) + '_' + str(self.eval_rate) + '_' + str(self.scaler_type) + '_' \
                + self.aug_params['mixup'] + '_' + self.aug_params['EMD'] + '_' + str(self.aug_params['imf_num']) + '_' + str(self.aug_params['EMD_rounds']) + '_' + str(self.aug_params['mixup_rate']) + '_' + str(self.aug_params['tol'])
        else:
            self.aug_parameters_str = ""
        self.aug_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'aug_air_quality_{}.npz'.format(self.aug_parameters_str))
        ...
        # 初始化
        self.data = None
        self.original_feature_name = {'X': 'float', 'y': 'float'}
        self.feature_name = {'X': 'float', 'y': 'float'}
        if self.loss_type == 'importance_weighting':
            self.feature_name = {'X': 'float', 'y': 'float', 'w': 'float'}
        elif self.loss_type == 'Group_DRO':
            self.feature_name = {'X': 'float', 'y': 'float', 'group_ids': 'int'}
        self.adj_mx = None
        self.scaler = None
        ...
        if os.path.exists(self.data_path + self.dyna_file + '.dyna'):
            self._load_dyna(self.dyna_file)
        else:
    ...
    def _load_geo(self):
        ...
        self.geo_coords = [eval(coord) for coord in list(geofile['coordinates'])]
    ...
    # def de_trend (self, original_data):
    ...
    def _load_dyna_3d(self, filename):
        ...
        raw_dataset_file = os.path.join('./libcity/cache/dataset_cache/',
                                            'raw_air_quality_{}.npy'.format(self.dataset))
        np.save(raw_dataset_file, data)
        
        # 这里可以进行de-trend和de-seasonalize
        # de-trend和de-seasonalize应该基于训练集进行，验证集和测试集使用基于训练集的结果，跟这里的流程还稍有不同
        # 如果做的话，这里就应该分出训练集了
        # train_seg_id = round(data.shape[0] * self.train_rate)
        # train_series = data[:train_seg_id]
        # 基于train_series求季节项和趋势项
        # 季节窗口大小-可按月份，求
    ...
    def _generate_input_data(self, df):
        ...
        y_offsets = np.sort(np.arange(self.output_offset, self.output_window + self.output_offset, 1))
    ...
    def _generate_input_data_with_seg(self, df, seg_size):
        ...
        for i in range(0, ranges_num):
            ...
            x = np.stack(x, axis=0)
            y = np.stack(y, axis=0)
            print('x,y one seg shape~', x.shape, y.shape)
            if i == 0:
                all_x = x.copy()
                all_y = y.copy()
            else:
    ...
    def _generate_data(self):
        ...
        param_str = str(self.dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' + str(self.output_offset)
        input_dataset_file = os.path.join('./libcity/cache/dataset_cache/',
                                            'input_air_quality_{}.npy'.format(param_str))
        output_dataset_file = os.path.join('./libcity/cache/dataset_cache/',
                                            'output_air_quality_{}.npy'.format(param_str))
    ...
    def _load_augmented_train_val_test(self):
        """
        加载之前保存在文件中的增强后的训练集、测试集、验证集
        Returns:
            tuple: tuple contains:
                original_x_train: (num_samples, input_length, ..., feature_dim) \n
                original_y_train: (num_samples, input_length, ..., feature_dim) \n
                x_train: (num_samples, input_length, ..., feature_dim) \n
                y_train: (num_samples, input_length, ..., feature_dim) \n
                x_val: (num_samples, input_length, ..., feature_dim) \n
                y_val: (num_samples, input_length, ..., feature_dim) \n
                ...
                y_test: (num_samples, input_length, ..., feature_dim)
        """
        self._logger.info('Loading ' + self.aug_file_name)
        cat_data = np.load(self.aug_file_name)
        original_x_train = cat_data['original_x_train']
        original_y_train = cat_data['original_y_train']
        ...
        self._logger.info("original_train\t" + "x: " + str(original_x_train.shape) + ", y: " + str(original_y_train.shape))
        self._logger.info("train\t" + "x: " + str(x_train.shape) + ", y: " + str(y_train.shape))
        self._logger.info("eval\t" + "x: " + str(x_val.shape) + ", y: " + str(y_val.shape))
        self._logger.info("test\t" + "x: " + str(x_test.shape) + ", y: " + str(y_test.shape))
        return original_x_train, original_y_train, x_train, y_train, x_val, y_val, x_test, y_test
    ...
    def _get_scalar(self, scaler_type, x_train, y_train):
        ...
        if scaler_type == "normal":
            ...
            x_train_max = np.amax(x_train, axis=(0,1,2))
            y_train_max = np.amax(y_train, axis=(0,1,2))
            x_train_min = np.amin(x_train, axis=(0,1,2))
            y_train_min = np.amin(y_train, axis=(0,1,2))
            
            scaler = MinMax01Scaler(
                maxx=np.maximum(x_train_max, y_train_max), minn=np.minimum(x_train_min, y_train_min))
    ...
    def get_data(self):
        ...
        if self.data is None:
            self.data = {}
            if self.augmented_data:
                # 加载增强的数据，包含原始的train和增强后的train
                original_x_train, original_y_train, x_train, y_train, x_val, y_val, x_test, y_test = self._load_augmented_train_val_test()
                print('data shape: ', original_x_train.shape, original_y_train.shape)
                print(x_train.shape, y_train.shape)
                print(x_val.shape, y_val.shape)
                print(x_test.shape, y_test.shape)
            elif self.cache_dataset and os.path.exists(self.cache_file_name):
        ...
        if self.augmented_data:
            original_x_train[..., :self.output_dim] = self.scaler.transform(original_x_train[..., :self.output_dim])
            original_y_train[..., :self.output_dim] = self.scaler.transform(original_y_train[..., :self.output_dim])
        ...
        if self.normal_external:
            if self.augmented_data:
                original_x_train[..., self.output_dim:] = self.ext_scaler.transform(original_x_train[..., self.output_dim:])
                original_y_train[..., self.output_dim:] = self.ext_scaler.transform(original_y_train[..., self.output_dim:])
        ...
        if self.loss_type == 'importance_weighting':
            # 加载weight文件
            weight_file = f"./libcity/weight_files/weight-{self.dataset}-{self.weight_file}.npy"
            print(f"Load weight file {weight_file}")
            weights = np.load(weight_file)
            weights_train = weights[:len(x_train), ...].tolist()
            weights_val = weights[len(x_train):(len(x_train)+len(x_val)), ...].tolist()
            weights_test = weights[(len(x_train)+len(x_val)):, ...].tolist()
            if self.augmented_data:
                original_train_data = list(zip(original_x_train, original_y_train))
            train_data = list(zip(x_train, y_train, weights_train))
            eval_data = list(zip(x_val, y_val, weights_val))
            test_data = list(zip(x_test, y_test, weights_test))
        elif self.loss_type == 'Group_DRO':
            # 加载weight文件
            group_file = f"./libcity/group_files/group-{self.dataset}-{self.group_file}.npy"
            print(f"Load group file {group_file}")
            group_labels = np.load(group_file)
            group_labels_train = group_labels[:len(x_train), ...].tolist()
            group_labels_val = group_labels[len(x_train):(len(x_train)+len(x_val)), ...].tolist()
            group_labels_test = group_labels[(len(x_train)+len(x_val)):, ...].tolist()
            if self.augmented_data:
                original_train_data = list(zip(original_x_train, original_y_train))
            train_data = list(zip(x_train, y_train, group_labels_train))
            eval_data = list(zip(x_val, y_val, group_labels_val))
            test_data = list(zip(x_test, y_test, group_labels_test))
        elif self.loss_type == 'ERM':
            if self.augmented_data:
                original_train_data = list(zip(original_x_train, original_y_train))
        ...
        # 转Dataloader
        if self.augmented_data:
            self.original_train_dataloader, self.train_dataloader, self.eval_dataloader, self.test_dataloader = generate_dataloader_aug(original_train_data, train_data, eval_data, test_data, self.feature_name, self.original_feature_name, self.batch_size, self.num_workers, shuffle=False, pad_with_last_sample=False)
        else:
            self.train_dataloader, self.eval_dataloader, self.test_dataloader = generate_dataloader(train_data, eval_data, test_data, self.feature_name, self.batch_size, self.num_workers, shuffle=False, pad_with_last_sample=False)
        
        # generate_dataloader_mixup(train_data, eval_data, test_data, self.feature_name,
                                # self.batch_size, self.num_workers, shuffle=False, pad_with_last_sample=False, EMD=self.EMD, imf_num=self.imf_num, mixup=self.mixup, aug_type=self.aug_type)
            
        self.num_batches = len(self.train_dataloader)
        if self.augmented_data: 
            return self.original_train_dataloader, self.train_dataloader, self.eval_dataloader, self.test_dataloader
        else:
