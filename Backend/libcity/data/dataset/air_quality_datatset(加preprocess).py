...
class AirQualityDataset(AbstractDataset):
    """
    交通状态预测数据集的基类。
    ...
    def __init__(self, config):
        ...
        # 新加参数
        self.time_unit = self.config.get('time_unit', '')
        self.preprocess = self.config.get('preprocess', '')
        # self.de_seasonalize = self.config.get('de_seasonalize', False)
        # self.de_trend = self.config.get('de_trend', False)
        ...
        self.cache_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'air_quality_{}.npz'.format(self.parameters_str))
        self.cache_preprocess_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'air_quality_preprocess_{}.npz'.format(self.parameters_str))
        ...
        self.season_means = None    # season cycle的day means dim: (365)
        self.trend_fit = None     # trend fit的多项式参数
        self.feature_name = {'X': 'float', 'y': 'float'}  # 此类的输入只有X和y
        self.adj_mx = None
        self.scaler = None
        self.scaler_preprocess = None
    ...
    def _distance_inverse(self):
        ...
        # max_dis = self.adj_mx.max()
        min_dis = self.adj_mx.min()
        self.adj_mx = min_dis / self.adj_mx
    ...
    def removeSC(self, x:np.ndarray, train_seg_id):
        '''
        Removes seasonal cycle from monthly data
        x (np.ndarray) - 3D (time,lat,lon) numpy array
        '''
        x_train = x[:train_seg_id, :, :self.output_dim]
        nt_train,nx,ny = x_train.shape # get array dimensions
        nyears = nt_train//365
        print('nyears', nyears)
        # get day_means
        day_means = np.mean(x_train[:(nyears*365)].reshape(nyears,365,nx,ny),axis=0)
        for m in range(12): #for each month
            x[m::365,:,:self.output_dim] = x[m::365,:,:self.output_dim] - day_means[m]
        print('day_means', day_means.shape)
        return x, day_means

    def detrend(self, x:np.ndarray, time:np.ndarray, train_seg_id):
        '''
        ...
        '''
        x_train = x[:train_seg_id, :, :self.output_dim]
        time_train = time[:train_seg_id]
        
        nt = x.shape[0]
        nt_train,nx,ny = x_train.shape
        xtemp_train = x_train.reshape(nt_train,nx*ny)
        p = np.polyfit(time_train, xtemp_train, deg=3)
        fit = p[0]*(time[:,np.newaxis] **3)+ p[1]*(time[:,np.newaxis]**2) + p[2]*(time[:,np.newaxis]) + p[3]
        fit = fit.reshape(nt,nx,ny)
        print('trend_fit', fit.shape)
        x[:, :, :self.output_dim] = x[:, :, :self.output_dim] - fit
        return x, fit
    ...
    def _load_dyna_3d(self, filename):
        ...
        # print(dynafile.head(10))
        # 求时间序列
        timesolts_list = list(dynafile['time'][:int(dynafile.shape[0] / len(self.geo_ids))])
        self.timesolts = [str(item) for item in timesolts_list]
        # print('timesolts', self.timesolts)
        self.idx_of_timesolts = dict()
        if not dynafile['time'].isna().any():  # 时间没有空值
            self.timesolts = list(map(lambda x: x.replace('T', ' ').replace('Z', '').replace('/', '-'), self.timesolts))
        ...
        # 将原始(未划分子序列)数据保存至文件
        raw_dataset_file = os.path.join('./libcity/cache/dataset_cache/',
                                            'raw_air_quality_{}.npy'.format(str(self.dataset)))
        np.save(raw_dataset_file, data)
        
        # 如果是天粒度数据，这里可以进行de-seasonalizede-trend和de-seasonalize
        # de-trend和de-seasonalize应该基于训练集进行，验证集和测试集使用基于训练集的结果，跟这里的流程还稍有不同
        preprocess_data = None
        self.train_seg_id = 0
        self.eval_seg_id = 0
        if self.time_unit == 'Day' and self.:     
            self.train_seg_id = round(data.shape[0] * self.train_rate)
            self.eval_seg_id = round(data.shape[0] * (self.train_rate+self.eval_rate))
            # 基于train_series求季节项和趋势项，并在整体序列中去除
            # 去季节化：返回处理后的数据，和减去的年均值，用于恢复预测的值，可以在验证和测试时与原始值对比
        
            de_season_data, self.season_means =  self.removeSC(data.copy(), self.train_seg_id)
            time_arr = np.arange(1, data.shape[0]+1)
            de_trend_data, self.trend_fit = self.detrend(de_season_data, time_arr, self.train_seg_id)
            preprocess_data = de_trend_data
        else:
            preprocess_data = data.copy()
            
        self._logger.info("Loaded file " + filename + '.dyna' + ', shape=' + str(data.shape))
        return data, preprocess_data
    ...
    def _generate_input_data_with_seg(self, df, seg_size):
        """
        根据全局参数`input_window`和`output_window`切分输入，产生模型需要的张量输入，
        即使用过去`input_window`长度的时间序列去预测未来`output_window`长度的时间序列
        ...
        Args:
            df(np.ndarray): 数据数组，shape: (len_time, ..., feature_dim)
            seg_size(int): 每隔seg_size个steps对时序进行划分
        ...
        Returns:
            tuple: tuple contains:
                ...
                y(np.ndarray): 模型输出数据，(epoch_size, output_length, ..., feature_dim)
        """
        ranges_num =  int(df.shape[0] / seg_size)
        all_x = []
        all_y = []
        for i in range(0, ranges_num):
            num_samples = seg_size
            # 预测用的过去时间窗口长度 取决于self.input_window
            x_offsets = np.sort(np.concatenate((np.arange(-self.input_window + 1, 1, 1),)))
            # 未来时间窗口长度 取决于self.output_window
            y_offsets = np.sort(np.arange(1, self.output_window + 1, 1))
            ...
            max_t = abs(num_samples - abs(max(y_offsets)))
            for t in range(min_t, max_t):
                x_t = df[i * seg_size + t + x_offsets, ...]
                y_t = df[i * seg_size + t + y_offsets, ...]
                x.append(x_t)
                y.append(y_t)
        ...
        print('x-shape ~ ', all_x_array.shape)
        print('y-shape ~ ', all_y_array.shape)
    ...
    def _generate_data(self):
        ...
        if self.load_external and os.path.exists(self.data_path + self.ext_file + '.ext'):  # 外部数据集
            ...
            ext_data = None
        x_list, y_list = [], []
        x_preprocess_list, y_preprocess_list = [], []
        for filename in data_files:
            df, df_preprocess = self._load_dyna(filename)  # (len_time, ..., feature_dim)
            print('df shape ~ ', df.shape)
            print('df_preprocess shape ~ ', df_preprocess.shape)
            if self.load_external:    # 默认为false
                df = self._add_external_information(df, ext_data)
                print('df external shape ~ ', df.shape)
            if self.time_unit == 'Day':
                x, y = self._generate_input_data(df)
                x_preprocess, y_preprocess = self._generate_input_data(df_preprocess)
            elif self.time_unit == 'Hour':
                seg_size = 744  # 744 = 31 * 24
                x, y = self._generate_input_data_with_seg(df, seg_size)
                x_preprocess, y_preprocess = self._generate_input_data_with_seg(df_preprocess, seg_size)
            print('x---shape', x.shape)
            print('y---shape', y.shape)
            ...
            y_preprocess_list.append(y_preprocess)
        x = np.concatenate(x_list)
        y = np.concatenate(y_list)
        x_preprocess = np.concatenate(x_preprocess_list)
        y_preprocess = np.concatenate(y_preprocess_list)
        input_dataset_file = os.path.join('./libcity/cache/dataset_cache/',
                                            'input_air_quality_{}.npy'.format(self.parameters_str))
        output_dataset_file = os.path.join('./libcity/cache/dataset_cache/',
                                            'output_air_quality_{}.npy'.format(self.parameters_str))
        np.save(input_dataset_file, x)
        np.save(output_dataset_file, y)
        
        self._logger.info("Dataset created")
        self._logger.info("x shape: " + str(x.shape) + ", y shape: " + str(y.shape))
        return x, y, x_preprocess, y_preprocess

    def _split_train_val_test(self, x, y, data_mode):
        """
        ...
        elif data_mode == 'preprocess':
            if self.cache_dataset:
                ensure_dir(self.cache_file_folder)
                np.savez_compressed(
                    self.cache_preprocess_file_name,
                    x_train=x_train,
                    y_train=y_train,
                    x_test=x_test,
                    y_test=y_test,
                ...
                self._logger.info('Saved at ' + self.cache_preprocess_file_name)
    ...
    def _generate_train_val_test(self):
        ...
        Returns:
            tuple: tuple contains:
                ...
                y_test: (num_samples, input_length, ..., feature_dim)
        """
        x, y, x_preprocess, y_preprocess = self._generate_data()
        return self._split_train_val_test(x, y, 'original'), self._split_train_val_test(x_preprocess, y_preprocess, 'preprocess')

    def _load_cache_train_val_test(self, data_mode):
        """
        加载之前缓存好的训练集、测试集、验证集
        ...
        if data_mode == 'original':
            ...
            self._logger.info('Loading ' + self.cache_preprocess_file_name)
            cat_data = np.load(self.cache_preprocess_file_name)
    ...
    def _get_scalar(self, scaler_type, x_train, y_train):
        ...
        if scaler_type == "normal":
            ...
            x_y_train = np.concatenate((x_train, y_train), axis=0)
            # print('x_y_train max --- ', np.amax(x_y_train, axis=(0,1,2)))
            scaler = MinMax01Scaler(
                maxx=np.amax(x_y_train, axis=(0,1,2)), minn=np.amin(x_y_train, axis=(0,1,2)))
    ...
    def get_data(self):
        ...
        x_preprocess_train, y_preprocess_train, x_preprocess_val, y_preprocess_val, x_preprocess_test, y_preprocess_test = [], [], [], [], [], []
        if self.data is None:
            self.data = {}
            if self.cache_dataset and os.path.exists(self.cache_file_name):
                x_train, y_train, x_val, y_val, x_test, y_test = self._load_cache_train_val_test('original')
                if self.time_unit == 'Day' and self.preprocess:
                    x_preprocess_train, y_preprocess_train, x_preprocess_val, y_preprocess_val, x_preprocess_test, y_preprocess_test = self._load_cache_train_val_test('preprocess')
                else:
                    x_preprocess_train = x_train
                    y_preprocess_train = y_train
                    x_preprocess_val = x_val
                    y_preprocess_val = y_val
                    x_preprocess_test = x_test
                    y_preprocess_test = y_test
            else:
                original_result, preprocess_result = self._generate_train_val_test()
                x_train, y_train, x_val, y_val, x_test, y_test = original_result
                x_preprocess_train, y_preprocess_train, x_preprocess_val, y_preprocess_val, x_preprocess_test, y_preprocess_test = preprocess_result
        ...
        self.scaler_preprocess = self._get_scalar(self.scaler_type,
                                       x_preprocess_train[..., :self.output_dim], y_preprocess_train[..., :self.output_dim])
        print('scalar min max ----- ', self.scaler.min, self.scaler.max)
        print('scaler_preprocess min max ----- ', self.scaler_preprocess.min, self.scaler_preprocess.max)
        self.ext_scaler = self._get_scalar(self.ext_scaler_type,
                                           x_train[..., self.output_dim:], y_train[..., self.output_dim:])
        print('ext_scaler min max ----- ', self.ext_scaler.min, self.ext_scaler.max)
        ...
        x_preprocess_train[..., :self.output_dim] = self.scaler_preprocess.transform(x_preprocess_train[..., :self.output_dim])
        y_preprocess_train[..., :self.output_dim] = self.scaler_preprocess.transform(y_preprocess_train[..., :self.output_dim])
        x_preprocess_val[..., :self.output_dim] = self.scaler_preprocess.transform(x_preprocess_val[..., :self.output_dim])
        y_preprocess_val[..., :self.output_dim] = self.scaler_preprocess.transform(y_preprocess_val[..., :self.output_dim])
        x_preprocess_test[..., :self.output_dim] = self.scaler_preprocess.transform(x_preprocess_test[..., :self.output_dim])
        y_preprocess_test[..., :self.output_dim] = self.scaler_preprocess.transform(y_preprocess_test[..., :self.output_dim])
        if self.normal_external:
            ...
            x_preprocess_train[..., self.output_dim:] = self.ext_scaler.transform(x_preprocess_train[..., self.output_dim:])
            y_preprocess_train[..., self.output_dim:] = self.ext_scaler.transform(y_preprocess_train[..., self.output_dim:])
            x_preprocess_val[..., self.output_dim:] = self.ext_scaler.transform(x_preprocess_val[..., self.output_dim:])
            y_preprocess_val[..., self.output_dim:] = self.ext_scaler.transform(y_preprocess_val[..., self.output_dim:])
            x_preprocess_test[..., self.output_dim:] = self.ext_scaler.transform(x_preprocess_test[..., self.output_dim:])
            y_preprocess_test[..., self.output_dim:] = self.ext_scaler.transform(y_preprocess_test[..., self.output_dim:])
        ...
        train_preprocess_data = list(zip(x_preprocess_train, y_preprocess_train))
        eval_preprocess_data = list(zip(x_preprocess_val, y_preprocess_val))
        test_preprocess_data = list(zip(x_preprocess_test, y_preprocess_test))
        # 转Dataloader
        self.train_dataloader, self.eval_dataloader, self.test_dataloader = \
            generate_dataloader(train_data, eval_data, test_data, self.feature_name,
                                self.batch_size, self.num_workers, pad_with_last_sample=False)
        self.train_preprocess_dataloader, self.eval_preprocess_dataloader, self.test_preprocess_dataloader = \
            generate_dataloader(train_preprocess_data, eval_preprocess_data, test_preprocess_data, self.feature_name,
                                self.batch_size, self.num_workers, pad_with_last_sample=False)
        self.num_batches = len(self.train_dataloader)
        return self.train_dataloader, self.eval_dataloader, self.test_dataloader, self.train_preprocess_dataloader, self.eval_preprocess_dataloader, self.test_preprocess_dataloader
    ...
    def get_data_feature(self):
        ...
        print("data feature scaler --- ", self.scaler)
        return {"scaler": self.scaler, "scaler_preprocess": self.scaler_preprocess, "season_means": self.season_means, "trend_fit": self.trend_fit, "time_unit": self.time_unit, "preprocess": self.preprocess, "adj_mx": self.adj_mx, "ext_dim": self.ext_dim, "num_nodes": self.num_nodes, "feature_dim": self.feature_dim, "output_dim": self.output_dim, "num_batches": self.num_batches}
