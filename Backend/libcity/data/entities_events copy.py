...
class Entities_Events():
    ...
    def compute_entity_match(self, entity1, entity2):
        ...
        if set(loc_ids_1).intersection(set(loc_ids_2)): return True
        for loc1 in loc_ids_1:
            for loc2 in loc_ids_2:
                cur_dis = self.haversine_distance(loc1, loc2)
        ...
        for phase_id, phase in enumerate(self.phases):
            ...
            for i, entities in enumerate(focus_entitys):
                if i == 0 or len(focus_entitys[i-1]) == 0:
                    ...
                    prev_centroids = [entity['centroid'] for entity in prev_entities]
                    curr_centroids = [entity['centroid'] for entity in entities]
                    distances = np.zeros((len(prev_entities), len(entities)))
                    for prev, prev_centroid in enumerate(prev_centroids):
                        for cur, curr_centroid in enumerate(curr_centroids):
                            
                            
                            distances[prev][cur] = self.haversine_distance(prev_centroid, curr_centroid)
                            
                    print('distances', distances)
                    entity_rel_mtx = np.zeros((len(prev_entities), len(entities)))
                    for prev in range(distances.shape[0]):
                        for cur in range(distances.shape[1]):
                            pair_dis = distances[prev][cur]
                            print('pair_dis', pair_dis, related_dis_th)
                            if pair_dis < related_dis_th:
                                entity_rel_mtx[prev][cur] = 1
