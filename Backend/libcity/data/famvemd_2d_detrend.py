...
def EMD2DmV(data, data_de, param):
    # 获取数据的维度
    ...
    while imf <= param['nimfs'] and stopflag:
        ...
        if len(np.nonzero(maxima)[0]) < 3 or len(np.nonzero(minima)[0]) < 3:
    ...
    for i in range(num_vars):
        Results['IO'][i], Results['Error'][i] = orth_index(data_de[i], IMF[i], Residue[i])
    
    if param['plot'] == 'on':
        plot_results(data, data_de, Results, param)
...
def identify_max_min(signal):
    ...
    maxima = signal >= B
    minima = signal <= C
    print(f"Maxima positions: {np.count_nonzero(maxima)}")  
    print(f"Minima positions: {np.count_nonzero(minima)}")
...
def plot_results(data, data_de, Results, param):
    trend = data - data_de
    # Plotting results
    num_vars = data.shape[0]
    nimfs = param['nimfs']
    
    fig, axs = plt.subplots(num_vars, nimfs + 4, figsize=(15, num_vars * 3))
    
    for var in range(num_vars):
        axs[var, 0].imshow(data[var], cmap='gray')
        axs[var, 0].set_title(f'Original Data {var+1}')
        axs[var, 1].imshow(data_de[var], cmap='gray')
        axs[var, 1].set_title(f'Detrend Data {var+1}')
        axs[var, 2].imshow(trend[var], cmap='gray')
        axs[var, 2].set_title(f'Trend {var+1}')
        for imf in range(nimfs):
            axs[var, imf + 3].imshow(Results['IMF'][var, :, :, imf]+trend[var], cmap='gray')
            axs[var, imf + 3].set_title(f'IMF {imf+1}')
        axs[var, nimfs + 3].imshow(Results['IMF'][var, :, :, nimfs-1], cmap='gray')
        axs[var, nimfs + 3].set_title(f'Original IMF {nimfs}')
...
x = np.arange(1, time_points+1) + np.random.randn(time_points)*2
y = np.arange(1, time_points+1) + np.random.randn(time_points)*2
z = np.arange(1, time_points+1) + np.random.randn(time_points)*2
...
multivariate_signal_de = detrend(multivariate_signal, axis=1)
...
results = EMD2DmV(multivariate_signal, multivariate_signal_de, param)
# 生成新的样本？？
