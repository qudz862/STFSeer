...
class SliceEvaluator():
    ...
    def process_scope_data(self):
        if self.data_scope == 'All_Data':
            # self.scope_residuals = np.array([[loc_list for loc_list in step_list] for step_list in self.residuals_series])
            self.scope_residuals = np.array(self.flatten_list(self.residuals_series))
            for attr in self.config['point_metadata']:
                # self.scope_metadata[attr] = np.array([[loc_list for loc_list in step_list] for step_list in self.metadata_series[attr]])
                self.scope_metadata[attr] = np.array(self.flatten_list(self.metadata_series[attr]))
            ...
            # self.scope_residuals = np.array([[loc_list for loc_list in step_list] for step_list in self.scope_residuals])
            self.scope_residuals = np.array(self.flatten_list(self.scope_residuals))
            for attr in self.config['point_metadata']:
                self.scope_metadata[attr] = [self.metadata_series[attr][ins_id] for ins_id in phase_ins_ids]
                # self.scope_metadata[attr] = np.array([[loc_list for loc_list in step_list] for step_list in self.scope_metadata[attr]])
                self.scope_metadata[attr] = np.array(self.flatten_list(self.scope_metadata[attr]))
    ...
    def range_division(self):
        ...
        ranges_infor_file = f"./point_slices/ranges_{self.config['dataset']}_{self.range_params['scope']}_{str(self.range_params['bin_num'])}_{self.range_params['sup_th']}_{str(err_th)}.json"
        if os.path.exists(ranges_infor_file):
            ...
            for attr in self.config['point_metadata']:
                ...
                val_step = self.range_params['step_len'][attr]
                # if var_id == 0:
                print('run supervised range division.')
                sup_th = self.range_params['sup_th']
                ranges_infor = seg_range_point(attr, self.metadata_ranges[attr], self.scope_metadata[attr], self.scope_residuals, val_step, sup_th, err_th)
                all_bin_edges = ranges_infor['seg_points']
                print('all_bin_edges', all_bin_edges)
                bin_edges = [round(bin_edge/val_step) * val_step for bin_edge in all_bin_edges]
                bin_edges = [round(num, 1) if isinstance(num, float) else num for num in bin_edges]
                self.metadata_ranges[attr]['bin_edges'] = bin_edges
                if self.range_params['scope'] == 'Hierarchy':
                    self.metadata_ranges[attr]['bins'] = ranges_infor['preorder_tree_range']
                    for i, bin_range in enumerate(self.metadata_ranges[attr]['bins']):
                        self.metadata_ranges[attr]['bins'][i] = [round(bin_edge/val_step) * val_step for bin_edge in bin_range]
                        self.metadata_ranges[attr]['bins'][i] = [float(Decimal(str(num)).quantize(Decimal(str(val_step)), rounding=ROUND_HALF_UP)) if val_step < 1 else num for num in self.metadata_ranges[attr]['bins'][i]]
                else:
                ...
                #         return [(lower_bound, upper_bound)] + left_ranges + right_ranges
                    ...
                    # else: 
                    #     self.metadata_ranges[attr]['bins'] = [[bin_edges[i], bin_edges[i+1]] for i in range(len(bin_edges)-1)]
                print(attr, 'bin_edges', bin_edges)
        ...
        for attr in self.config['point_metadata']:
            ...
            # residuals_flatten = np.array(self.flatten_list(self.residuals_series))
            bin_edges = self.metadata_ranges[attr]['bin_edges']
            for cur_range in self.metadata_ranges[attr]['bins']:
                ...
                cur_range_infor['agg_range'] = False
                if bin_edges.index(cur_range[1]) - bin_edges.index(cur_range[0]) > 1:
                    cur_range_infor['agg_range'] = True
    ...
    def subgroup_mining(self):
        ...
        for i, subset in enumerate(scope_subsets_iterative):
            ...
            if 'contain_subsets' in subset:
                contain_subsets_index.append(i)
                for j, individual_subset in enumerate(scope_subsets_iterative[i]['contain_subsets']):
                    individual_subset['subset_id'] = str(i+1) + '-' + str(j+1)
                    self.slices_indices[individual_subset['subset_id']] = individual_subset.pop('indices')
        
        while i < len(scope_subsets_iterative):
            if 'contain_subsets' in scope_subsets_iterative[i]:
                tmp_subset = scope_subsets_iterative[i]
                scope_subsets_iterative = scope_subsets_iterative[:i] + tmp_subset['contain_subsets'] + scope_subsets_iterative[i:]
                i += len(tmp_subset['contain_subsets'])
            i += 1
