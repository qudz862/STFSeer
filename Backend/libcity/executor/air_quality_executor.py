...
class AirQualityExecutor(AbstractExecutor):
    def __init__(self, config, model, data_feature):
        ...
        self._scaler_preprocess = self.data_feature.get('scaler_preprocess')
    ...
    def _build_train_loss(self):
        ...
        def func(batch):
            ...
            if self.data_feature['time_unit'] == 'Day' and self.data_feature['preprocess']:
                y_true = self._scaler_preprocess.inverse_transform(y_true[..., :self.output_dim])
                y_predicted = self._scaler_preprocess.inverse_transform(y_predicted[..., :self.output_dim])
            else: 
    ...
    def evaluate(self, test_dataloader, mode_label):
        """
        ...
        for batch in test_dataloader:
            ...
            # 计算损失
            loss = loss_function(output[..., :self.output_dim], target[..., :self.output_dim])
            # 反向传播
            loss.backward()
            # 提取输入数据的梯度
            saliency_map = input_data.grad
        ...
        saliency_maps = np.concatenate(saliency_maps, axis=0)  # concatenate on batch
        # print('y_preds shape ~ ---',y_preds.shape)
        outputs = {'prediction': y_preds, 'truth': y_truths, 'saliency': saliency_maps}
        # filename = \
        #     time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime(time.time())) + '_' \
        #     + self.config['model'] + '_' + self.config['dataset'] + mode_label + '_predictions.npz'
        filename = self.config['model'] + '_' + self.config['dataset'] + '_' + mode_label + '_predictions.npz'
        np.savez_compressed(os.path.join(self.evaluate_res_dir, filename), **outputs)
        self.evaluator.clear()
        self.evaluator.collect({'y_true': torch.tensor(y_truths), 'y_pred': torch.tensor(y_preds)})
        test_result = self.evaluator.save_result(self.evaluate_res_dir, mode_label)
        
        # TODO:可以在这里直接做处理，生成flag和level的数据，并且合并train、eval和test
        
        
        # return test_result
        return y_truths, y_preds

    def evaluate_preprocess(self, test_dataloader, mode_label, dataset):
        """
        use model to test data
        ...
        # 计算tend的fit
        # dataset.season_means, dataset.trend_fit
        start_time = 0
        if mode_label == 'train':
            start_time = dataset.input_window
        elif mode_label == 'valid':
            start_time = dataset.train_seg_id + dataset.input_window
        elif mode_label == 'test':
            start_time = dataset.eval_seg_id + dataset.input_window
        with torch.no_grad():
            self.model.eval()
            # self.evaluator.clear()
            y_truths = []
            ...
            for batch in test_dataloader:
                ...
                # print(self._scaler.min, self._scaler.max)
                y_true = self._scaler_preprocess.inverse_transform(batch['y'][..., :self.output_dim])
                y_pred = self._scaler_preprocess.inverse_transform(output[..., :self.output_dim])
                y_true = y_true.cpu().numpy()
                y_pred = y_pred.cpu().numpy()
                for i in range(y_true.shape[0]):
                    for j in range(y_true.shape[1]):
                        time_id = start_time + i + j
                        y_true[i][j] += dataset.season_means[time_id%365, :, :self.output_dim]
                        y_pred[i][j] += dataset.season_means[time_id%365, :, :self.output_dim]
                        y_true[i][j] += dataset.trend_fit[time_id, :, :self.output_dim]
                        y_pred[i][j] += dataset.trend_fit[time_id, :, :self.output_dim]
                # print(y_true)
    ...
    def train(self, train_dataloader, eval_dataloader):
        ...
        for epoch_idx in range(self._epoch_num, self.epochs):
            ...
            losses = self._train_epoch(train_dataloader, epoch_idx, self.loss_type, self.loss_func)
            t1 = time.time()
            ...
            t2 = time.time()
            val_loss = self._valid_epoch(eval_dataloader, epoch_idx, self.loss_type, self.loss_func)
            end_time = time.time()
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, loss_type='ERM', loss_func=None):
        """
        完成模型一个轮次的训练
        ...
        if loss_func is not None:
            cur_loss_func = loss_func
        elif loss_type == 'ERM':
            cur_loss_func = self.model.calculate_loss
        elif loss_type == 'importance_weighting':
            cur_loss_func = self.model.calculate_loss_IW
        elif loss_type == 'Group_DRO':
            cur_loss_func = self.model.calculate_loss_GDRO
        
        losses = []
        for batch in train_dataloader:
            self.optimizer.zero_grad()
            batch.to_tensor(self.device)
            loss = cur_loss_func(batch)
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, loss_type='ERM', loss_func=None):
        """
        完成模型一个轮次的评估
