...
class STGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.train_mode = config.get('stgcn_train_mode', 'full')  # or full
    ...
    def calculate_loss_GDRO(self, batch):
        group_labels = batch['group_ids']
        if self.train_mode.lower() == 'quick':
            if self.training:  # 训练使用t+1时间步的loss
                y_true = batch['y'][:, 0:1, :, :]  # (batch_size, 1, num_nodes, feature_dim)
                y_predicted = self.forward(batch)  # (batch_size, 1, num_nodes, output_dim)
                group_labels = batch['group_ids'][:, 0:1, :]
            else:  # 其他情况使用全部时间步的loss
                y_true = batch['y']  # (batch_size, output_length, num_nodes, feature_dim)
                y_predicted = self.predict(batch)  # (batch_size, output_length, num_nodes, output_dim)
                group_labels = batch['group_ids']
        ...
        y_true_groups = {group.item(): self._scaler.inverse_transform(y_true[..., :self.output_dim][group_labels == group]) for group in unique_groups}
        y_predicted_groups = {group.item(): self._scaler.inverse_transform(y_predicted[..., :self.output_dim][group_labels == group]) for group in unique_groups}
        
        group_losses = [loss.masked_mse_torch(y_predicted_groups[group.item()], y_true_groups[group.item()]) for group in unique_groups]
        max_loss = max(group_losses)
        
        return max_loss
    
    def calculate_loss_IW(self, batch):
        batch_weights = batch['w']
        if self.train_mode.lower() == 'quick':
            if self.training:  # 训练使用t+1时间步的loss
                y_true = batch['y'][:, 0:1, :, :]  # (batch_size, 1, num_nodes, feature_dim)
                y_predicted = self.forward(batch)  # (batch_size, 1, num_nodes, output_dim)
                batch_weights = batch['w'][:, 0:1, :]
            else:  # 其他情况使用全部时间步的loss
                y_true = batch['y']  # (batch_size, output_length, num_nodes, feature_dim)
                y_predicted = self.predict(batch)  # (batch_size, output_length, num_nodes, output_dim)
                batch_weights = batch['w']
        ...
        return loss.masked_mse_torch_weighting(y_predicted, y_true, batch_weights)
