...
watch (() => inforStore.cur_event_pcp_dims, (oldValue, newValue) => {
  console.log('inforStore.cur_event_pcp_dims', inforStore.cur_event_pcp_dims);
  drawPhasesPCP()
})

watch (() => inforStore.sel_event_types, (oldValue, newValue) => {
  console.log('inforStore.sel_event_types', inforStore.sel_event_types);
  drawPhasesPCP()
}, { deep: true })


let focused_phase_attr = ref('change_area')

function drawPhasesPCP() {
  let dim_objs = {}
  d3.select('#events-pcp-svg').selectAll('*').remove()
  let margin_left = 44, margin_right = 30, margin_top = 18, margin_bottom = 10
  let main_w = 1240, main_h = 140
  ...
  let events_objs = inforStore.st_phase_events.phases[inforStore.cur_focused_model][inforStore.cur_phase_sorted_id].phase_event_objs
  if (inforStore.sel_event_types.length > 0) {
    events_objs = events_objs.filter(item => inforStore.sel_event_types.includes(item.area_type))
    console.log('events_objs.length', events_objs.length);
  }
  ...
  // 为每个轴定义scale
  let yScales = {}
  console.log('cur_event_pcp_dims', Object.keys(inforStore.cur_event_pcp_dims), inforStore.cur_event_pcp_dims);
  Object.keys(inforStore.cur_event_pcp_dims).forEach(dim => {
    console.log(dim);
    ...
    dim_objs[dim].bin_edges = inforStore.cur_event_pcp_dims[dim]
  ...
  for (let key in dim_objs) {
    ...
    for (let i = 0; i < events_objs.length; ++i) {
      let phase_obj = events_objs[i]
      let bin_idx = -1 
      ...
      if ((focus_bin_idx != -1) && focus_bin_idx < all_stack_data[key][bin_idx].length)
  ...
  let tmp_dims = Object.keys(inforStore.cur_event_pcp_dims).slice()
  tmp_dims.push('tmp_dim')
  let xScale = d3.scalePoint()
    .domain(tmp_dims)
    .range([0, main_w])
  let axisInterval = xScale(Object.keys(inforStore.cur_event_pcp_dims)[1]) - xScale(Object.keys(inforStore.cur_event_pcp_dims)[0])

  // 添加轴标签
  let textElements = pcp_svg.selectAll(".axis-label")
    .data(Object.keys(inforStore.cur_event_pcp_dims))
    .enter().append("text")
        ...
        .join("rect")
          ...
          .on('click', (e, d) => {
            ...
            if (cur_rect.attr('stroke') != 'none'){
              ...
              inforStore.cur_sel_event_steps = []
              return
            }
            ...
            let all_phases_indices = events_objs.map((item,index) => index)
            let filtered_phase_objs = events_objs.filter(item => item.selected == 1)
            console.log('filtered_phase_objs', filtered_phase_objs);
            inforStore.cur_sel_event_steps = filtered_phase_objs.map(item => item.step_id)
            pcp_svg.selectAll(".line").remove()
            lines.data(filtered_phase_objs)
              .join('path')
              .attr("class", "line")
              .attr("d", d => d3.line()(Object.keys(inforStore.cur_event_pcp_dims).map(dim => {
  ...
  // 添加路径
  let sel_events_objs = events_objs.filter(item => item.selected == 1)
  let lines = pcp_svg.selectAll(".line")
    .data(sel_events_objs)
    .join("path")
    .attr("class", "line")
    .attr("d", d => d3.line()(Object.keys(inforStore.cur_event_pcp_dims).map(dim => {
      let dim_bins = dim_objs[dim].bins
    ...
    brushes[dim] = d3.brushY()
      ...
      .on("end", (e) => {
        ...
        if (selection) {
          ...
          console.log('cur_filter_conditions.value[cur_dim]', cur_filter_conditions.value[cur_dim]);
        ...
        let filtered_phase_objs = events_objs.filter(item => item.selected == 1)
        if (filtered_phase_objs.length == events_objs.length) inforStore.cur_sel_event_steps = []
        else inforStore.cur_sel_event_steps = filtered_phase_objs.map(item => item.step_id)

        pcp_svg.selectAll(".line").remove()
        if (sel_cnt != events_objs.length) {
          lines.data(filtered_phase_objs)
            .join('path')
            .attr("class", "line")
            .attr("d", d => {
              return d3.line()(Object.keys(inforStore.cur_event_pcp_dims).map(dim => {
                let dim_bins = dim_objs[dim].bins
                let y_id = -1
                for (let i = 0; i < dim_bins.length; ++i) {
                  if (d[dim] < dim_bins[i][1]) {
